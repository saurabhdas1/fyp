...
public class SequenceDatabaseConverter {
	...
	String output; // the path of the file to be written to disk in PASD format
	int lineCount =0; // the number of sequences in the input file
	BufferedWriter writer; // to write the output file

	/**
	 * This method converts a sequence database from a given format to the PASD format.
	 ...
	 * @param output the path of the file to be written to disk in PASD format
	...
	public void convert(String input, String output, Formats inputFileformat, int lineCount, Charset charset) throws IOException {
		...
		}else if(inputFileformat.equals(Formats.PASD_TRANSACTION_DB)){
	...
	/**
	 * This method convert a transaction database in PASD format to a sequence database in PASD format
	...
	/**
	 * This method convert a file from the SNAKE format to PASD format
	 */
	...
	/**
	 * This method convert a file from the BMS format to PASD format
	 * @param charset the charset (encoding) of the input file  (e.g "UTF-8")
	 *   or null if you want to use the default text encoding.
	 */
	...
	/**
	 * This method convert a file from the CSV format to PASD format
	 * @param charset the charset (encoding) of the input file  (e.g "UTF-8")
	 *   or null if you want to use the default text encoding.
	 */
	...
	/**
	 * This method convert a file from the KOSARAK format to PASD format
	 * @param charset the charset (encoding) of the input file if it is a text file (e.g "UTF-8")
	 *   or null if you want to use the default text encoding or if it is not a text file.
	 */
	...
	/**
	 * This method convert a file from the IBM GENERATOR format to PASD format
	 * 
	 */
	private void convertIBMGenerator() {
		...
		try {
			...
			//  but in PASD format there is one).
	...
	/**
	 * This method convert a text file to a sequence database in PASD format and
	 * return a map of key = item id  value = corresponding attribute value. This 
	 * method is to be used by the GUI version of PASD that need to keep the mapping
	...
	/**
	 * This method convert a file from the TEXT format to the PASD format.
	 * 
	...
	private Map<Integer, String> convertTEXT(boolean returnMapItemIDWord, Charset charset) throws IOException {
		...
		// It is used only if returnMapItemIDValue is set to true.  This is used by the GUI of PASD
